...
class ReadParquetSpecsOp : public OpKernel {
 public:
  explicit ReadParquetSpecsOp(OpKernelConstruction* context) : OpKernel(context) {
    env_ = context->env();
  }
...
REGISTER_KERNEL_BUILDER(Name("ReadParquetSpecs").Device(DEVICE_CPU),
                        ReadParquetSpecsOp);
