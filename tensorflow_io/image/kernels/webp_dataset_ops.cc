  ...
  class Dataset : public DatasetBase {
   ...
   private:
	...
	if (current_file_index_ < dataset()->filenames_.size()) {
          ...
          Tensor value_tensor(ctx->allocator({}), DT_UINT8, {config.output.height, config.output.width, channel});
          const uint64_t num_bytes = channel * config.output.width * (uint64_t)config.output.height;
          std::memcpy(reinterpret_cast<char*>(value_tensor.flat<uint8_t>().data()), config.output.u.RGBA.rgba, num_bytes * sizeof(uint8_t));
