  ...
  void Compute(OpKernelContext* context) override {
    ...
    for (uint32 i = 0; i < height; i++) {
      ...
      for (uint32 j = 0; j < width; j++) {
        ...
        const uint8* data = &input_tensor->flat<uint8>().data()[i * width * channels + j * channels];
        switch (channels)
        {
        case 3:
          // RGB => BGR
          ...
          break;
        default:
          OP_REQUIRES(context, false, errors::InvalidArgument("unsupported number of channels: ", channels));
        }
      }
...
REGISTER_KERNEL_BUILDER(Name("EncodeBmp").Device(DEVICE_CPU),
                        EncodeBmpOp);
