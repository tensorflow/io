    ...
    with tf.name_scope("SQLIODataset"):
      ...
      if spec is None:
        ...
        spec = {
            field.numpy().decode(): tf.TensorSpec(
                [None],
                tf.as_dtype(dtype.numpy()),
                field.numpy().decode()) for (
                    field, dtype) in zip(fields, dtypes)}
      else:
      ...
      def f(index):
        return tf.nest.pack_sequence_as(
            spec, core_ops.io_sql_iterable_read(
                resource, index, fields, dtypes))

      dataset = dataset.map(f)
      dataset = dataset.unbatch()
      self._dataset = dataset
      super(SQLIODataset, self).__init__(
