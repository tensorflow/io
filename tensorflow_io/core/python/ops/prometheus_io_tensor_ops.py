...
class PrometheusIOTensor(io_tensor_ops._SeriesIOTensor): # pylint: disable=protected-access
  ...
  def __init__(self,
    ...
    with tf.name_scope("PrometheusIOTensor") as scope:
      ...
      resource = core_golang_ops.prometheus_readable_init(
          query, metadata=metadata,
          container=scope, shared_name="%s/%s" % (query, uuid.uuid4().hex))
      index_shape, index_dtype = core_golang_ops.prometheus_readable_spec(
          resource, "index")
      value_shape, value_dtype = core_golang_ops.prometheus_readable_spec(
          resource, "value")
      ...
      index = io_tensor_ops.BaseIOTensor(
          ...
          _PrometheusIOTensorFunction(
              core_golang_ops.prometheus_readable_read,
