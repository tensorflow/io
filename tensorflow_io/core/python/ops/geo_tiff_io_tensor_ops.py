...
class GeoTIFFIOTensor(io_tensor_ops._CollectionIOTensor): # pylint: disable=protected-access
  """GeoTIFFIOTensor"""
  ...
  def __init__(self,
               filename,
               internal=False):
    with tf.name_scope("GeoTIFFIOTensor"):
      # GeoTIFF can fit into memory so load GeoTIFF first
      data = tf.io.read_file(filename)
      shapes, dtypes = core_ops.io_decode_geo_tiff_info(data)
      ...
      elements = [
          io_tensor_ops.TensorIOTensor(
              core_ops.io_decode_geo_tiff(data, i),
              internal=internal) for i in columns]
      super(GeoTIFFIOTensor, self).__init__(
