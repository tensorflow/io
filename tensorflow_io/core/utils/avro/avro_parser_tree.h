...
public:
  ...
  // Creates all parser nodes with the given namespace and for the given keys with
  // their types
...
private:
  ...
  // Note, that this method only creates value parsers for non-primitive avro types,
  // e.g. attributes, maps, arrays, arrays with filters
  Status CreateValueParser(AvroParserUniquePtr& value_parser, const string& infix,
    const string& user_name) const;

  // Creates the value parser for primitive avro types for the given user name and data type
  ...
  // This is used to mark the outer-most dimension as begun -- before any element is added
  static Status AddBeginMarks(std::map<string, ValueStoreUniquePtr>* key_to_value);

  // Add a finish mark to all value stores
  // This is used to mark the outer-most dimension as finished -- before any element is added
  ...
  // Handles the absolute reference notation with symbol @ where the symbol needs to be removed
  ...
  // Checks if the name contains a filter expression, e.g. 'does.not.matter[abc=efg].more.does.not.matter'.
  // If a filter is present this method returns the left-hand-side name and the right-hand-side name
  static bool ContainsFilter(string* lhs_name, string* rhs_name, const string& name);

  // Is the infix a filter expression? E.g. '[abc=efg]'?
  // If this is true returns the left-hand-side name and the right-hand-side name
  ...
  // Is this infix is an attribute name? -- must be a valid avro name, regex [A-Za-z_]
  // See https://avro.apache.org/docs/1.8.2/spec.html#names
  ...
  // Is this infix a branch? -- must be [:boolean|:int|:long|:float|:double|:bytes|:string]
