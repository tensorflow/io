  ...
  Status Init(const std::vector<string>& input,
    ...
    for (size_t i = 0; i < metadata.size(); i++) {
      if (metadata[i].find("conf.eof") == 0) {
        ...
        if (parts.size() != 2) {
          return errors::InvalidArgument("invalid bounded configuration: ",
                                         metadata[i]);
        }
        eof_ = (parts[1] != "0");
        ...
        if (parts.size() != 2 || !strings::safe_strto64(parts[1], &timeout_)) {
          return errors::InvalidArgument("invalid timeout configuration: ",
                                         metadata[i]);
        }
    ...
    *record_read = 0;
    if (start != index_) {
      ...
      if (start > 0) {
        ...
        return errors::InvalidArgument(
            "dataset can not seek to a random location");
      }
      // Case 2: start = 0
      ...
      if (offset_tail_ < 0) {
        return errors::InvalidArgument(
            "stream dataset can not return back to 0");
      }
    ...
    while (consumer_.get() != nullptr && index_ < stop) {
      ...
      if (message->err() == RdKafka::ERR_NO_ERROR) {
        ...
        value->flat<string>()(index_) = std::string(
      ...
      if (message->err() == RdKafka::ERR__PARTITION_EOF) {
        LOG(INFO) << "Partition reach EOF, current offset: "
                  << subscription_->offset();
        if (offset_tail_ >= 0) {
          return errors::Internal("received EOF unexpected: ",
                                  message->errstr());
        }
