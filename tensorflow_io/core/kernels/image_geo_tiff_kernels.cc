...
TIFF*
XTIFFStreamOpen(const char* name, std::istream *is)
{
	const char* mode = "rm";
	TIFF*	tif;
...
class DecodeGeoTIFFInfoOp : public OpKernel {
 public:
  explicit DecodeGeoTIFFInfoOp(OpKernelConstruction* context) : OpKernel(context) {}
...
class DecodeGeoTIFFOp : public OpKernel {
 public:
  explicit DecodeGeoTIFFOp(OpKernelConstruction* context) : OpKernel(context) {}
...
REGISTER_KERNEL_BUILDER(Name("IO>DecodeGeoTiffInfo").Device(DEVICE_CPU),
                        DecodeGeoTIFFInfoOp);
REGISTER_KERNEL_BUILDER(Name("IO>DecodeGeoTiff").Device(DEVICE_CPU),
                        DecodeGeoTIFFOp);
