...
static const std::map<avro::Type, DataType> avro_to_tf_datatype = {
  {avro::AVRO_INT, DT_INT32},
  {avro::AVRO_LONG, DT_INT64},
  {avro::AVRO_STRING, DT_STRING},
  {avro::AVRO_BYTES, DT_STRING},
  {avro::AVRO_FLOAT, DT_FLOAT},
  {avro::AVRO_DOUBLE, DT_DOUBLE},
 ...
 * Decoders are invoked in a multithreaded context(controlled by `num_parallel_calls`).
 * Therefore the implementations must be threadsafe.
...
class DecoderBase {
  public:
    ...
    virtual Status operator ()(avro::DecoderPtr&,
                               std::vector<Tensor>&,
                               sparse::ValueBuffer&,
...
std::unique_ptr<DecoderBase> CreateFeatureDecoder(const avro::NodePtr&, const Metadata&);
