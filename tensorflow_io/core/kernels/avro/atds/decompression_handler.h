#ifndef TENSORFLOW_DATA_CORE_KERNELS_AVRO_ATDS_DECOMPRESSION_HANDLER_H_
#define TENSORFLOW_DATA_CORE_KERNELS_AVRO_ATDS_DECOMPRESSION_HANDLER_H_
...
class DecompressionHandler {
 public:
  ...
  // Adapted from https://github.com/apache/avro/blob/release-1.9.1/lang/c++/impl/DataFile.cc#L58
  ...
  avro::InputStreamPtr decompressSnappyCodec(AvroBlock& block) {
    ...
    if (!snappy::Uncompress(compressed.data(),
            len - 4, &uncompressed)) {
    ...
    uint32_t c = crc();
    if (checksum != c) {
        throw avro::Exception(boost::format("Checksum did not match for Snappy compression: Expected: %1%, computed: %2%") % checksum % c);
    }
    block.content = uncompressed;
    block.byte_count = uncompressed.size();
    block.codec = avro::NULL_CODEC;
    uint8_t* dt = reinterpret_cast<uint8_t*>(block.content.data() + block.read_offset);
    return avro::memoryInputStream(dt, block.content.size() - block.read_offset);
  }
  #endif
  ...
  avro::InputStreamPtr decompressDeflateCodec(AvroBlock& block) {
    ...
    stream.push(boost::iostreams::basic_array_source<char>(block.content.data(), block.content.size()));
...
#endif // TENSORFLOW_DATA_CORE_KERNELS_AVRO_ATDS_DECOMPRESSION_HANDLER_H_