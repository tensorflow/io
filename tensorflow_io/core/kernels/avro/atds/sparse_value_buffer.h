...
template<>
inline const std::vector<int>& GetValueVector(const ValueBuffer& buffer, size_t index) {
  return buffer.int_values[index];
}

template<>
inline const std::vector<long>& GetValueVector(const ValueBuffer& buffer, size_t index) {
  return buffer.long_values[index];
}

template<>
inline const std::vector<float>& GetValueVector(const ValueBuffer& buffer, size_t index) {
  return buffer.float_values[index];
}

template<>
inline const std::vector<double>& GetValueVector(const ValueBuffer& buffer, size_t index) {
  return buffer.double_values[index];
}

template<>
inline const std::vector<string>& GetValueVector(const ValueBuffer& buffer, size_t index) {
  return buffer.string_values[index];
}

template<>
inline const std::vector<bool>& GetValueVector(const ValueBuffer& buffer, size_t index) {
  return buffer.bool_values[index];
}

inline Status FillIndicesTensor(const std::vector<long>& buffer,
                                Tensor& tensor,
                                size_t offset) {
  void* dest = reinterpret_cast<void*>(
    reinterpret_cast<long*>(tensor.data()) + offset);
...
inline Status FillValuesTensor(const sparse::ValueBuffer& buffer,
  ...
  void* dest = reinterpret_cast<void*>(
    reinterpret_cast<T*>(tensor.data()) + offset);
...
inline Status FillValuesTensor(const sparse::ValueBuffer& buffer,
  ...
  switch (dtype) {
    ...
    case DT_INT64: {
      return FillValuesTensor<long>(buffer, values_tensor, values_index, offset);
    }
    case DT_FLOAT: {
      return FillValuesTensor<float>(buffer, values_tensor, values_index, offset);
    }
    case DT_DOUBLE: {
      return FillValuesTensor<double>(buffer, values_tensor, values_index, offset);
    }
    case DT_STRING: {
      return FillValuesTensor<string>(buffer, values_tensor, values_index, offset);
    }
    case DT_BOOL: {
      return FillValuesTensor<bool>(buffer, values_tensor, values_index, offset);
    }
    default: {
