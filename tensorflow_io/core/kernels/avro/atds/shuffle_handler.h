...
class ShuffleHandler {
 public:
  ...
  void SampleBlocks(size_t batch_size,
                    bool shuffle,
                    std::vector<std::unique_ptr<AvroBlock>>& blocks){
    size_t i = 0;
    size_t block_size = blocks.size();
    // LOG(INFO) << "shuffle batch size " << batch_size << " shuffle block size: " << block_size;
    if (!shuffle) {
      size_t j = 0;
      while (i < batch_size) {
        auto& random_block = blocks[j];
        random_block->num_to_decode = std::min(random_block->object_count - random_block->num_decoded,
                                               static_cast<int64_t>(batch_size - i));
        i+= random_block->num_to_decode;
        if ((random_block->num_decoded + random_block->num_to_decode)==random_block->object_count) {
          j++;
        }
      }
    } else {
      while (i < batch_size) {
        ...
        // LOG(INFO) << "shuffle block size " << block_size << " block_id: " << block_id << " actual block size: " << blocks.size();
        auto& random_block = blocks[block_id];
        int64 remaining = random_block->object_count - random_block->num_decoded - random_block->num_to_decode;
        if (remaining > 0) {
          // Decode the whole block when it has less than 1/10 of the undecoded records.
          // It is to quickly recycle the almost decoded blocks.
          int64 decode_all_threshold = random_block->object_count / 10;
          size_t decode_num = 1;
          if (remaining <= decode_all_threshold) {
            decode_num = std::min(static_cast<size_t>(remaining), batch_size - i);
          }
    ...
    for (size_t k = 0; k < block_size; k++) {
      ...
      // LOG(INFO) << "block " << k << " object count: " << blocks[k]->object_count << " counts: " << blocks[k]->counts
      //            << " num_decoded: " << blocks[k]->num_decoded << " num_to_decode: " << blocks[k]->num_to_decode;
    }
  }
  ...
  void ResetRngs()
    ...
    generator_ =
      std::make_unique<random::SingleSampleAdapter<random::PhiloxRandom>>(parent_generator_.get());
...
private:
  ...
  std::unique_ptr<random::SingleSampleAdapter<random::PhiloxRandom>> generator_ TF_GUARDED_BY(*mu_);
};
