...
TEST(FileBufferInputStreamTest, SINGLE_BUFFER) {
  ...
  std::unique_ptr<tensorflow::RandomAccessFile> raf = absl::make_unique<MockRandomAccessFile>(content, 8);
  int64 buffer_size = 8;
...
TEST(FileBufferInputStreamTest, READ_PAST_BUFFER) {
  ...
  std::unique_ptr<tensorflow::RandomAccessFile> raf = absl::make_unique<MockRandomAccessFile>(content, 16);
  int64 buffer_size = 8;
  FileBufferInputStream stream(raf.get(), buffer_size);
  const uint8_t* data;
  size_t len = 3;
...
class StringOutputStream : public avro::OutputStream {
  public:
    ...
    bool next(uint8_t **data, size_t *len) {
      ...
      *data = reinterpret_cast<uint8_t*>(const_cast<char*>(&(buf_->c_str())[pos_]));
...
Status AvroBlockReaderTest(char* content, size_t byte_count) {
  std::unique_ptr<tensorflow::RandomAccessFile> raf = absl::make_unique<MockRandomAccessFile>(content, byte_count);
  std::unique_ptr<AvroBlockReader> reader = absl::make_unique<AvroBlockReader>(raf.get(), BUFFER_SIZE);
  AvroBlock blk;
  return reader->ReadBlock(blk);
}

template<typename T>
void AvroBlockReaderTest(char* expected_content, int64_t object_count, size_t expected_byte_count,
    avro::ValidSchema& schema, const std::vector<T>& data) {
  string buf;
  std::unique_ptr<avro::OutputStream> os = absl::make_unique<StringOutputStream>(&buf);
  ...
  std::unique_ptr<tensorflow::RandomAccessFile> raf = absl::make_unique<MockRandomAccessFile>(const_cast<char*>(buf.c_str()), buf.capacity());
  std::unique_ptr<AvroBlockReader> reader = absl::make_unique<AvroBlockReader>(raf.get(), BUFFER_SIZE);
  ...
  tensorflow::atds::AssertValueEqual(expected_content, blk.content.c_str(), blk.byte_count);
}

/*
 * These bytes assume the Avro file format specified here: https://avro.apache.org/docs/1.9.1/spec.html#Object+Container+Files
 * Bytes were manually generated via:
 ...
 *
 * If avro file format changes, this byte array will need to be regenerated, and test cases modified
 * to change different byte locations in the array.
 */
static constexpr size_t BYTEARRAY_SIZE = 268;
static constexpr char WELLFORMED_CONTENT[] = {
  0x4f, 0x62, 0x6a, 0x01, 0x04, 0x16, 0x61, 0x76, 0x72, 0x6f, 0x2e, 0x73, 0x63, 0x68, 0x65, 0x6d,                                                                             // Obj...avro.schem
  0x61, static_cast<char>(0xec), 0x02, 0x7b, 0x22, 0x74, 0x79, 0x70, 0x65, 0x22, 0x3a, 0x22, 0x72, 0x65, 0x63, 0x6f,                                                          // a..{"type":"reco
  0x72, 0x64, 0x22, 0x2c, 0x22, 0x6e, 0x61, 0x6d, 0x65, 0x22, 0x3a, 0x22, 0x72, 0x6f, 0x77, 0x22,                                                                             // rd","name":"row"
  0x2c, 0x22, 0x66, 0x69, 0x65, 0x6c, 0x64, 0x73, 0x22, 0x3a, 0x5b, 0x7b, 0x22, 0x6e, 0x61, 0x6d,                                                                             // ,"fields":[{"nam
  0x65, 0x22, 0x3a, 0x22, 0x64, 0x65, 0x6e, 0x73, 0x65, 0x5f, 0x31, 0x64, 0x22, 0x2c, 0x22, 0x74,                                                                             // e":"dense_1d","t
  0x79, 0x70, 0x65, 0x22, 0x3a, 0x7b, 0x22, 0x74, 0x79, 0x70, 0x65, 0x22, 0x3a, 0x22, 0x61, 0x72,                                                                             // ype":{"type":"ar
  0x72, 0x61, 0x79, 0x22, 0x2c, 0x22, 0x69, 0x74, 0x65, 0x6d, 0x73, 0x22, 0x3a, 0x22, 0x69, 0x6e,                                                                             // ray","items":"in
  0x74, 0x22, 0x7d, 0x7d, 0x2c, 0x7b, 0x22, 0x6e, 0x61, 0x6d, 0x65, 0x22, 0x3a, 0x22, 0x64, 0x65,                                                                             // t"}},{"name":"de
  0x6e, 0x73, 0x65, 0x5f, 0x32, 0x64, 0x22, 0x2c, 0x22, 0x74, 0x79, 0x70, 0x65, 0x22, 0x3a, 0x7b,                                                                             // nse_2d","type":{
  0x22, 0x74, 0x79, 0x70, 0x65, 0x22, 0x3a, 0x22, 0x61, 0x72, 0x72, 0x61, 0x79, 0x22, 0x2c, 0x22,                                                                             // "type":"array","
  0x69, 0x74, 0x65, 0x6d, 0x73, 0x22, 0x3a, 0x7b, 0x22, 0x74, 0x79, 0x70, 0x65, 0x22, 0x3a, 0x22,                                                                             // items":{"type":"
  0x61, 0x72, 0x72, 0x61, 0x79, 0x22, 0x2c, 0x22, 0x69, 0x74, 0x65, 0x6d, 0x73, 0x22, 0x3a, 0x22,                                                                             // array","items":"
  0x69, 0x6e, 0x74, 0x22, 0x7d, 0x7d, 0x7d, 0x5d, 0x7d, 0x14, 0x61, 0x76, 0x72, 0x6f, 0x2e, 0x63,                                                                             // int"}}}]}.avro.c
  0x6f, 0x64, 0x65, 0x63, 0x08, 0x6e, 0x75, 0x6c, 0x6c, 0x00, static_cast<char>(0xe1), 0x26, 0x18, 0x0e, static_cast<char>(0xc9), static_cast<char> (0xbe),                   // odec.null..&....
  0x5a, static_cast<char>(0x8c), 0x5f, static_cast<char>(0xe0), static_cast<char>(0xcd), 0x5c, 0x62, static_cast<char>(0xc2), 0x3f, 0x05, 0x02, 0x1e, 0x06, 0x02, 0x04, 0x06, // Z._..\b.?.......
  0x00, 0x04, 0x04, 0x08, 0x0a, 0x00, 0x04, 0x0c, 0x0e, 0x00, 0x00, static_cast<char>(0xe1), 0x26, 0x18, 0x0e, static_cast<char>(0xc9),                                       // ............&...
  static_cast<char>(0xbe), 0x5a, static_cast<char>(0x8c), 0x5f, static_cast<char>(0xe0), static_cast<char>(0xcd), 0x5c, 0x62, static_cast<char>(0xc2), 0x3f, 0x05, 0x0a       // .Z._..\b.?..
};
...
TEST(AvroBlockReaderTest, MALFORMED_MAGIC) {
  ...
  ASSERT_STREQ("Invalid data file. Magic does not match.", expected_exception.what());
}
...
TEST(AvroBlockReaderTest, SYNC_MARKER_MISMATCH) {
  ...
  sync_marker_mismatch[218] = 0xe2; // Change second byte of sync marker from 0xe1 to 0xe2
...
TEST(AvroBlockReaderTest, DENSE_2D) {
  ...
  tensorflow::atds::ATDSSchemaBuilder schema_builder = tensorflow::atds::ATDSSchemaBuilder();
  ...
  avro::OutputStreamPtr out_stream = tensorflow::atds::EncodeAvroGenericDatum(datum);
  ...
  AvroBlockReaderTest<avro::GenericDatum>((char*) expected_content, 1, expected_len, schema, {datum});
}
...
TEST(AvroBlockReaderTest, SPARSE_2D) {
  ...
  tensorflow::atds::AddSparseValue<long>(datum1, feature_name, {{1, 2}, {3, 4}}, {5, 6});
  tensorflow::atds::AddSparseValue<long>(datum2, feature_name, {{7, 8}, {9, 10}}, {11, 12});
  std::vector<avro::GenericDatum> records = {datum1, datum2};
  avro::OutputStreamPtr out_stream = tensorflow::atds::EncodeAvroGenericData(records);
  avro::InputStreamPtr in_stream = avro::memoryInputStream(*out_stream);
  const uint8_t* expected_content;
  size_t expected_len;
  in_stream->next(&expected_content, &expected_len);
  AvroBlockReaderTest<avro::GenericDatum>((char*) expected_content, 2, expected_len, schema, {datum1, datum2});
}
