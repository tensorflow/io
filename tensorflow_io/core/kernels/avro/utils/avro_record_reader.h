...
// https://avro.apache.org/docs/1.7.7/api/cpp/html/index.html -- Note, in 1.8.2
// the display of examples is broken
namespace tensorflow {
namespace data {

class AvroReaderOptions {
 public:
  static AvroReaderOptions CreateReaderOptions() {
    return AvroReaderOptions(64 * 1024, "");
  }
  int64 buffer_size;
...
class AvroRecordReader {
 public:
  explicit AvroRecordReader(RandomAccessFile* file,
 ...
 public:
  ...
  explicit SequentialAvroRecordReader(
      RandomAccessFile* file, const AvroReaderOptions& options =
                                  AvroReaderOptions::CreateReaderOptions());
