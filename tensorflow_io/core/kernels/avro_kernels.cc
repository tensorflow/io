...
class AvroInputStream : public avro::SeekableInputStream {
 public:
  AvroInputStream(tensorflow::RandomAccessFile* file) : file_(file) {}
  ...
  void backup(size_t len) override { byte_count_ -= len; }
  void skip(size_t len) override { byte_count_ += len; }
  void seek(int64_t position) override { byte_count_ = position; }
  size_t byteCount() const override { return byte_count_; }
...
class ListAvroColumnsOp : public OpKernel {
 public:
  explicit ListAvroColumnsOp(OpKernelConstruction* context)
      : OpKernel(context) {
    env_ = context->env();
  }
  ...
  void Compute(OpKernelContext* context) override {
    ...
    for (int i = 0; i < reader_schema.root()->names(); i++) {
      const avro::GenericDatum& field =
          record.field(reader_schema.root()->nameAt(i));
      string dtype;
      switch (field.type()) {
        case avro::AVRO_BOOL:
          dtype = "bool";
...
REGISTER_KERNEL_BUILDER(Name("IO>ListAvroColumns").Device(DEVICE_CPU),
                        ListAvroColumnsOp);
REGISTER_KERNEL_BUILDER(Name("IO>ReadAvro").Device(DEVICE_CPU), ReadAvroOp);

}  // namespace

class AvroReadable : public IOReadableInterface {
 public:
  AvroReadable(Env* env) : env_(env) {}
  ...
  Status Init(const std::vector<string>& input,
    ...
    for (size_t i = 0; i < reader_schema_.root()->names(); i++) {
      ...
      switch (field.type()) {
        ...
        case avro::AVRO_ENUM:
          ...
          break;
        default:
          return errors::InvalidArgument("Avro type unsupported: ",
                                         field.type());
      }
    ...
    reader_.reset(new avro::DataFileReader<avro::GenericDatum>(
        std::move(reader_stream_), reader_schema_));
    ...
    while (offset < file_size_) {
      ...
      TF_RETURN_IF_ERROR(
          file_->Read(offset, buffer.size(), &result, &buffer[0]));
      std::unique_ptr<avro::InputStream> in =
          avro::memoryInputStream((const uint8_t*)result.data(), result.size());
      decoder->init(*in);
      long items = decoder->decodeLong();

      total += static_cast<int64>(items);
      positions_.emplace_back(
          std::pair<int64, int64>(static_cast<int64>(items), offset));
