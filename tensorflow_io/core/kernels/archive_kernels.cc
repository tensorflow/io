...
public:
  ArchiveRandomAccessFile(Env* env, const string& filename, const string& optional_memory) : SizedRandomAccessFile(env, filename, optional_memory) {}
...
class ReadArchiveEntriesOp : public OpKernel {
 public:
  explicit ReadArchiveEntriesOp(OpKernelConstruction* context) : OpKernel(context) {
    env_ = context->env();
...
REGISTER_KERNEL_BUILDER(Name("ReadArchiveEntries").Device(DEVICE_CPU),
                        ReadArchiveEntriesOp);
