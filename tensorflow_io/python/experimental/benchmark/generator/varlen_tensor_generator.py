...
DIM_DISTRIBUTION_TO_RANGE = {
  DimensionDistribution.LARGE_DIM: (5, 10)
}
...
class VarLenTensorGeneratorBase(Generator):
  ...
  def __init__(self, spec, dim_dist):
    ...
    if not isinstance(spec, tf.SparseTensorSpec):
      raise TypeError("Input spec must be a tf.SparseTensorSpec in VarLenTensorGenerator "
                      f"but found {spec}")

    if not isinstance(dim_dist, DimensionDistribution):
      raise TypeError(f"dim_dist must be a DimensionDistribution but found {dim_dist}")
  ...
  def _get_dim(self):
    if self._dim_dist == DimensionDistribution.ONE_DIM:
      ...
      raise ValueError(f"Found unsupported dimension distribution {self._dim_dist}")

  def _get_shape(self):
    return [dim if dim is not None else self._get_dim() for dim in self.spec.shape.as_list()]
...
class IntVarLenTensorGenerator(VarLenTensorGeneratorBase):
  ...
  def __init__(self, spec, dim_dist=DimensionDistribution.ONE_DIM):
    ...
    if spec.dtype not in [tf.int32, tf.int64]:
      raise TypeError("IntVarLenTensorGenerator can only generate tf.sparse.SparseTensor with "
...
class FloatVarLenTensorGenerator(VarLenTensorGeneratorBase):
  ...
  def __init__(self, spec, dim_dist=DimensionDistribution.ONE_DIM):
    ...
    if spec.dtype not in [tf.float32, tf.float64]:
      raise TypeError("FloatVarLenTensorGenerator can only generate tf.sparse.SparseTensor with "
...
class WordVarLenTensorGenerator(VarLenTensorGeneratorBase):
  ...
  def __init__(self, spec, dim_dist=DimensionDistribution.ONE_DIM,
    ...
    if spec.dtype is not tf.string:
      raise TypeError("WordVarLenTensorGenerator can only generate tf.sparse.SparseTensor with "
                      f"dtype in tf.string but found {spec.dtype}.")

    if avg_length < 1:
      raise ValueError("WordVarLenTensorGenerator must have positive avg_length"
...
class BoolVarLenTensorGenerator(VarLenTensorGeneratorBase):
  ...
  def __init__(self, spec, dim_dist=DimensionDistribution.ONE_DIM):
    ...
    if spec.dtype is not tf.bool:
      raise TypeError("BoolVarLenTensorGenerator can only generate tf.sparse.SparseTensor with "
