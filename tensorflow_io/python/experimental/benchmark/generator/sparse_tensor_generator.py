...
class SparseTensorGeneratorBase(Generator):
  ...
  def __init__(self, spec, num_values):
    ...
    if not isinstance(spec, tf.SparseTensorSpec):
      raise TypeError("Input spec must be a tf.SparseTensorSpec in SparseTensorGenerator "
                      f"but found {spec}")

    if not isinstance(num_values, (int, ValueDistribution)):
      raise TypeError(f"num_values must be an int or ValueDistribution but found {num_values}")
  ...
  def _get_shape(self):
    ...
    shape = [None] * np.random.randint(1, 5) if self.spec.shape == None else self.spec.shape
...
class IntSparseTensorGenerator(SparseTensorGeneratorBase):
  ...
  def __init__(self, spec, num_values):
    ...
    if spec.dtype not in [tf.int32, tf.int64]:
      raise TypeError("IntSparseTensorGenerator can only generate tf.sparse.SparseTensor with "
...
class FloatSparseTensorGenerator(SparseTensorGeneratorBase):
  ...
  def __init__(self, spec, num_values):
    ...
    if spec.dtype not in [tf.float32, tf.float64]:
      raise TypeError("FloatSparseTensorGenerator can only generate tf.sparse.SparseTensor with "
...
class WordSparseTensorGenerator(SparseTensorGeneratorBase):
  ...
  def __init__(self, spec, num_values, avg_length=5):
    ...
    if spec.dtype is not tf.string:
      raise TypeError("WordSparseTensorGenerator can only generate tf.sparse.SparseTensor with "
                      f"dtype in tf.string but found {spec.dtype}.")

    if avg_length < 1:
      raise ValueError("WordSparseTensorGenerator must have positive avg_length"
...
class BoolSparseTensorGenerator(SparseTensorGeneratorBase):
  ...
  def __init__(self, spec, num_values):
    ...
    if spec.dtype is not tf.bool:
      raise TypeError("BoolSparseTensorGenerator can only generate tf.sparse.SparseTensor with "
