...
def get_canonical_name(tensor_type, rank, dtype, num_records, partitions):
  ...
  if not isinstance(tensor_type, TensorType):
    raise TypeError("Input tensor_type must be a TensorType enum"
                    f" but found {tensor_type}")
...
def _init_data_source_registry(registry):
  ...
  dense_generators = [
    IntTensorGenerator, IntTensorGenerator, FloatTensorGenerator,
    FloatTensorGenerator, WordTensorGenerator, BoolTensorGenerator
  ]
  for cls, dtype in zip(dense_generators, dtypes):
    for shape in shapes:
      name = get_canonical_name(TensorType.DENSE, rank=len(shape), dtype=dtype,
          num_records=SMALL_NUM_RECORDS, partitions=SINGLE_PARTITION)
  ...
  sparse_generators = [
    IntSparseTensorGenerator, IntSparseTensorGenerator, FloatSparseTensorGenerator,
  ...
  for cls, dtype in zip(sparse_generators, dtypes):
    for shape in shapes:
      if len(shape) == 0:
        ...
        continue
      name = get_canonical_name(TensorType.SPARSE, rank=len(shape), dtype=dtype,
          num_records=SMALL_NUM_RECORDS, partitions=SINGLE_PARTITION)
  ...
  varlen_generators = [
    IntVarLenTensorGenerator, IntVarLenTensorGenerator, FloatVarLenTensorGenerator,
    FloatVarLenTensorGenerator, WordVarLenTensorGenerator, BoolVarLenTensorGenerator
  ]
  varlen_shapes = [[None]]
  for cls, dtype in zip(varlen_generators, dtypes):
    for shape in varlen_shapes:
      rank = len(shape)
      name = get_canonical_name(TensorType.VARLEN, rank=rank, dtype=dtype,
          num_records=SMALL_NUM_RECORDS, partitions=SINGLE_PARTITION)
