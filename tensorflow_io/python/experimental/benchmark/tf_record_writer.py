...
import os
from tensorflow_io.python.experimental.benchmark.generator.tensor_generator import TensorGeneratorBase
from tensorflow_io.python.experimental.benchmark.generator.sparse_tensor_generator import SparseTensorGeneratorBase
...
class TFRecordWriter(FileWriter):
      ...
      with tf.io.TFRecordWriter(file_path) as file_writer:
        for _ in range(num_records):
          ...
          for feature_name in scenario:
            ...
            self._add_tensor_to_features(generator, feature_name, feature, features)
          record_bytes = tf.train.Example(
            features=tf.train.Features(feature=features)
          ).SerializeToString()
          file_writer.write(record_bytes)
  ...
  def _add_tensor_to_features(self, generator, feature_name, tensor, features):
    ...
    if isinstance(spec, tf.TensorSpec):
      ...
      if issubclass(generator.get_generator_cls(), VarLenTensorGeneratorBase) \
        ...
        self._add_dense_tensor_to_features(feature_name, tf.sparse.to_dense(tensor).numpy(), tensor.dtype, features)
      elif issubclass(generator.get_generator_cls(), SparseTensorGeneratorBase) \
          and spec.shape.is_fully_defined():
        self._add_sparse_tensor_to_features(
            feature_name, tensor, features)
      else:
        raise ValueError("SparseTensorSpec shape must be either a 1D varlen tensor from VarLenTensorGenerator "
  ...
  def create_tf_example_parser_fn(self, data_source, with_batch=False):
    ...
    scenario = data_source.scenario
    feature_description = {
      name: self._build_tf_example_parsing_config(
        name, scenario[name]
  ...
  def _build_tf_example_parsing_config(self, name, generator):
    ...
    if isinstance(spec, tf.TensorSpec):
      ...
      if issubclass(generator.get_generator_cls(), VarLenTensorGeneratorBase) \
        ...
        rank = len(spec.shape)
        index_key = [
          f"{index_name}{dim}" for dim in range(rank)
        ]
