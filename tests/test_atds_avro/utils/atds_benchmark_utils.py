...
def get_features_from_data_source(writer, data_source):
  ...
  features = {
    feature_name: writer._get_atds_feature(scenario[feature_name]) for feature_name in scenario
  }
  return features
...
def run_atds_benchmark(tensor_type, rank, dtype, num_records, partitions, batch_size, benchmark):
...
def run_atds_benchmark_from_data_source(data_source, batch_size, benchmark,
