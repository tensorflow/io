...
from tensorflow_io.python.experimental.benchmark.generator.tensor_generator import \
   IntTensorGenerator, FloatTensorGenerator, WordTensorGenerator
from tensorflow_io.python.experimental.benchmark.generator.sparse_tensor_generator import \
   FloatSparseTensorGenerator, ValueDistribution
...
@pytest.mark.benchmark(group="jrps",)
def test_jrps_benchmark_data():
    scenario = {
        ...
        "sparse_1d_float_large": FloatSparseTensorGenerator(
            tf.SparseTensorSpec([50001], tf.dtypes.float32), ValueDistribution.SINGLE_VALUE),
        "dense_0d_float": FloatTensorGenerator(tf.TensorSpec([], tf.dtypes.float32)),
        "dense_1d_float_large_1": FloatTensorGenerator(tf.TensorSpec([200], tf.dtypes.float32)),
        ...
        "dense_1d_float_large_2": FloatTensorGenerator(tf.TensorSpec([200], tf.dtypes.float32)),
        "dense_1d_float_small_1": FloatTensorGenerator(tf.TensorSpec([2], tf.dtypes.float32)),
        "dense_1d_float_large_3": FloatTensorGenerator(tf.TensorSpec([200], tf.dtypes.float32)),
        "dense_1d_float_small_2": FloatTensorGenerator(tf.TensorSpec([2], tf.dtypes.float32)),
        "dense_1d_float_small_3": FloatTensorGenerator(tf.TensorSpec([2], tf.dtypes.float32)),
        ...
        "sparse_1d_float_small_2": FloatSparseTensorGenerator(
            tf.SparseTensorSpec([3], tf.dtypes.float32), ValueDistribution.SINGLE_VALUE),
        "dense_1d_float_large_4": FloatTensorGenerator(tf.TensorSpec([200], tf.dtypes.float32)),
        "dense_1d_float_small_4": FloatTensorGenerator(tf.TensorSpec([1], tf.dtypes.float32)),
        "dense_0d_string_1": WordTensorGenerator(tf.TensorSpec([], tf.dtypes.string), avg_length=24),
        "dense_0d_int_2": IntTensorGenerator(tf.TensorSpec([], tf.dtypes.int32)),
        "dense_0d_string_2": WordTensorGenerator(tf.TensorSpec([], tf.dtypes.string), avg_length=24),
        "dense_0d_long": IntTensorGenerator(tf.TensorSpec([], tf.dtypes.int64)),
    }
    num_partitions = 10
    data_source = DataSource(scenario=scenario, num_records=SMALL_NUM_RECORDS, partitions=num_partitions)
