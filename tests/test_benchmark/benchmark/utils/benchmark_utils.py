...
MIXED_TYPES_SCENARIO = {
  ...
  "int32_0d_dense": IntTensorGenerator(
  ...
  "bool_1d_dense": BoolTensorGenerator(
...
def create_tf_record_dataset(filenames, parse_function, batch_size, file_parallelism=None,
...
def run_tf_record_benchmark(tensor_type, rank, dtype, num_records, partitions, batch_size, benchmark):
...
def run_tf_record_benchmark_from_data_source(data_source, batch_size, benchmark, file_parallelism=None,
    shuffle_buffer_size=0, rounds=100):
  with TFRecordWriter() as writer:
    ...
    dataset = create_tf_record_dataset(filenames, parse_function, batch_size, file_parallelism=file_parallelism,
