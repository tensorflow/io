...
def fixture_hdf5(request):
  ...
  with h5py.File(filename, 'w') as f:
    ...
    f.create_dataset('complex', data=np.asarray(complex_data, np.complex))
  args = filename
  def func(args):
    """func"""
    ...
    complex = tfio.IODataset.from_hdf5(args, dataset='/complex')
    return tf.data.Dataset.zip(
        (u8, u16, u32, u64, i8, i16, i32, i64, f32, f64, ss, complex))
  expected = list(zip(
      ...
      np.asarray(complex_data, np.complex).tolist()))
  def fin():
    shutil.rmtree(tmp_path)
  request.addfinalizer(fin)
