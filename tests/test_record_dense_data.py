...
def test_dataset(batch_size, num_parallel_reads, test_setup):
    """This tests the dataset with a combination of num_parallel_calls from 1-4(inclusive),
    and num_parallel_reads 4-8(inclusive). The staggered ranges ensures num_parallel_reads >= num_parallel_calls

    """
    # More meaningful numbers are
    #  batch_size=4096
    #  reader_buffer_size=16*1024
...
def test_vary_parallel_reads_calls(num_parallel_reads,
    ...
    """
    os.environ["AVRO_PARSER_NUM_MINIBATCHES"] = str(num_minibatches)
...
def test_vary_batch_block(batch_size, block_length, test_setup_vanilla, benchmark):
    os.environ["AVRO_PARSER_NUM_MINIBATCHES"] = "5"
    # https://pytest-benchmark.readthedocs.io/en/latest/pedantic.html
    count = benchmark.pedantic(
        target=test_setup_vanilla.benchmark_data.perf_dataset,
        args=[batch_size],
...
def setup_function(function):
    if os.environ.get("AVRO_PARSER_NUM_MINIBATCHES"):
        del os.environ["AVRO_PARSER_NUM_MINIBATCHES"]
