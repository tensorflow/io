...
if [ "$#" -ne 2 ]; then
  echo "Usage: $0 start|stop <bigtable container name>" >&2
  exit 1
fi
...
if [ "$action" == "start" ]; then
    ...
    docker run -d --rm --net=host --name=$container -v $base:/v -w /v google/cloud-sdk:236.0.0 bash -x -c 'gcloud beta emulators bigtable start'
