...
run_test() {
  ...
  (cd tests && LD_PRELOAD=$(clang -print-file-name=libclang_rt.asan-x86_64.so) DYLD_INSERT_LIBRARIES=$(clang -print-file-name=libclang_rt.asan-x86_64.so) $entry -m pytest --benchmark-disable -v --import-mode=append --forked --numprocesses=auto --dist loadfile $(find . -type f \( -iname "test_*.py" \)))
}
