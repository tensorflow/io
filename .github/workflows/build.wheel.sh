...
run_test() {
  ...
  $entry -m pip install -q pytest pytest-benchmark "pytest-xdist[psutil]" boto3 fastavro avro-python3 scikit-image pandas pyarrow==3.0.0 google-cloud-pubsub==2.1.0 google-cloud-bigtable==1.6.0 google-cloud-bigquery-storage==1.1.0 google-cloud-bigquery==2.3.1 google-cloud-storage==1.32.0 PyYAML==5.3.1 azure-storage-blob==12.8.1
  (cd tests && $entry -m pytest --benchmark-disable -v --import-mode=append -n auto --dist loadscope $(find . -type f \( -iname "test_*_v1.py" \)))
  (cd tests && $entry -m pytest --benchmark-disable -v --import-mode=append -n auto --dist loadscope $(find . -type f \( -iname "test_*.py" ! \( -iname "test_*_v1.py" -o -iname "test_bigquery.py" \) \)))
  # GRPC and test_bigquery tests have to be executed separately because of https://github.com/grpc/grpc/issues/20034
  (cd tests && $entry -m pytest --benchmark-disable -v --import-mode=append -n auto --dist loadscope $(find . -type f \( -iname "test_bigquery.py" \)))
}
